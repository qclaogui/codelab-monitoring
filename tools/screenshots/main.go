...
func main() {
	...
	level.Info(logger).Log("msg", "Starting mixin screenshots generator")
...
func takeScreenshot(ctx context.Context, dashboard Dashboard, logger log.Logger) error {
	...
	// Build dashboard URL
	params := url.Values{}
	params.Set("var-datasource", "Metrics")
	...
	dashboardURL := fmt.Sprintf("http://host.docker.internal:3000/d/%s?%s&kiosk", dashboard.UID, params.Encode())
	...
	actions := []struct {
		...
		{"Sleep", chromedp.Sleep(10 * time.Second)},
